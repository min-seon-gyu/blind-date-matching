...
import { createProfile } from '@/api/participant'
...
export default function ProfileSetupPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      if (accessToken && user) {
        setAuth(accessToken, { ...user, hasProfile: true })
      }
  ...
  const isValid = name && age && gender && job && height && mbti

  return (
    <div className="max-w-md mx-auto min-h-screen bg-[#FFF5F5]">
      {/* Header */}
      ...
      <form onSubmit={handleSubmit} className="px-4 pb-8 space-y-6">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <span className="text-xs text-gray-500">프로필 사진 (선택)</span>
        </div>
